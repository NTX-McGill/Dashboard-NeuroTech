...
function GuidedRecorder({ recording, onKey, onPrompt, stopRecording }) {
	const fingers = [
		...
		let interval = setInterval(() => {
			if (recording) {
				...
				if (progress >= 100) {
					...
					if (promptCount === totalPromptCount)
						stopRecording(true);
	...
	}, [fingers, onPrompt, progress, prompt, recording, promptCount, stopRecording]);
	...
	return (
		<div>
			{recording && <>
				<Typography variant="h6">On prompt #{promptCount} of {totalPromptCount}</Typography>
				<LinearProgress
					style={{ height: 18, marginBottom: 40, }}
					variant="determinate"
					value={promptCount / totalPromptCount * 100}
				/>
