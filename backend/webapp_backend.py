...
file_path = "" # Defining this globally gets rid of errors
calibration_done = False # Defining this globally gets rid of errors
...
def prompt():
    ...
    finger = request.args.get(
        'finger')  # pinkie | ring finger | middle finger | index finger

    with open(file_path, 'a') as f:
        f.write(
            ', '.join([datetime, timestamp, 'prompt_end', hand, finger, '']) +
            '\n')
...
def custom_prompt():
    ...
    with open(file_path, 'a') as f:
        if (not hand is None) and (not finger is None) and (not key is None):
            f.write(', '.join(
                [datetime, timestamp, 'prompt_end', hand, finger, key]) + '\n')
        else:
...
def new_session():
    ...
    prompts = request.args.get(
        'prompts')  # prompts, defined if mode is guided or in air
    ...
    file_path = os.path.join(session_filepath,
                             datetime.replace(':', '-') + '.txt')
    print(file_path)
...
@app.route('/continue-calibration')
...
@app.route('/calibration-done')
def calibration_done_route():
    global calibration_done

    return {"done": calibration_done}
