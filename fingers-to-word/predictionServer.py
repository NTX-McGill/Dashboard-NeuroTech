...
@sio.on('finger down')
def get_finger_down(sid,finger_number):
    #-demo purposes--
    print ("Key " + finger_number + " pressed")
    finger_number = letters_to_finger_numbers[finger_number]
    #----------------

    print ("Finger " + finger_number + " pressed")
    finger_number_queue.append(finger_number)
...
async def get_finger_number():
    ...
    return finger_number_queue.popleft()
...
async def interactive_mode(word_groupings, server_mode=False, finger_mode=False):
    ...
    while True:
        ...
        # type space to select the first word (thumb)
            ...
            await handle_word_capture(word_groupings[finger_word][0])
