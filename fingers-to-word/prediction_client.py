...
def interactive_mode(word_groupings, server_mode=False, finger_mode=False):
    ...
    def handle_word_selection_mode(potential_words):
        nonlocal finger_word, sentence, word_selection_mode
    ...
    @sio.on('Finger')
    def handleFingerPrediction(finger_number):
        nonlocal finger_word, sentence, word_selection_mode, potential_words
        print((finger_number))
        finger_number = str(finger_number)
    ...
    # while True:
        ...
        # type 0 to enter word selection mode (left squeeze)
            ...
            if finger_word in word_groupings:
                potential_words = word_groupings[finger_word]
...
if __name__ == "__main__":
    ...
    sio.connect("http://localhost:4002")
    sio.wait()
