...
export default class App extends Component {
  ...
  let color_id = 0;
    ...
    // probs data is array of arrays
    // data is feature data, 
    function updateChart(newData) { // probs array with shape 10
      ...
      for (let i = 0; i < probsData.length; i++) { //x
        for (let j = 0; j < numFingers; j++) { // y
          ...
          let maxRow = probsData.map(function(row){ return Math.max.apply(Math, row); });
          let max = Math.max.apply(null, maxRow);
          color_id = Math.floor(probsData[i][j] / max * (colors.length - 1));
